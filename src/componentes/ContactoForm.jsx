...
function ContactoForm(){
    ...
    return(
        <div className={`contenedor-formulario ${cambioModo == 'modoClaro'? 'contenedor-formulario-modo-claro':'contenedor-formulario-modo-oscuro'}`}>

            <form className='formulario' action="https://formspree.io/f/manldnor" method="POST" target='_blank' onSubmit={eventoSubmit}>

                <label htmlFor='nombre'>{`${lenguaje.nombreFormulario}:`}</label>
                <input id='nombre' name='Nombre' type='text' placeholder={`${lenguaje.placeholderNombre}`} value={form.Nombre} onChange={eventoChange} required/>
                {error.Nombre && <p id='error'>{error.Nombre}</p>}

                <label htmlFor='email'>{`${lenguaje.correoFormulario}:`}</label>
                <input id='email' name='Email' type='email' placeholder={`${lenguaje.placeholderCorreo}`} value={form.Email} onChange={eventoChange} required/>
                {error.Email && <p id='error'>{error.Email}</p>}

                <label htmlFor='motivo'>{`${lenguaje.motivoFormulario}:`}</label>
                <textarea id='motivo' name='Motivo' placeholder={`${lenguaje.placeholderMotivo}...`} value={form.Motivo} onChange={eventoChange} required></textarea>
                {error.Motivo && <p id='error'>{error.Motivo}</p>}

                <button className='enviar' type='submit'>{`${lenguaje.botonEnviar}`}</button>
            </form>
